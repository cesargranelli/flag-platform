...
- **SelectableChip** (#290): variação compacta (raio 10) para grupos com muitas opções (ex.: faixa etária), gap 8 em wrap; altura ~34px (padding 16×8), métricas estáveis entre estados (borda sempre 1px, peso fixo w500). Não selecionado: fundo `surface` + borda `black` · **selecionado**: fundo `primary` + borda `primary` + texto **`black`**, tipografia 13/17 (`footerLink`)
