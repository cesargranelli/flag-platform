...
- **Regra de conteúdo sobre primário (#294)**: conteúdo (**texto e ícone**) sobre preenchimento `primary` (#FD6B22) e sobre `success` (#4FBF67) usa **BRANCO** — decisão do produto; nota de acessibilidade: contraste branco/laranja ≈ 2,9:1, abaixo de WCAG AA (~4,5:1), aceito conscientemente para estados de seleção
- **Navegação por sessões (#323) — cards centralizados, SEM numeração**: botões de seleção de sessão em formato de `Card` + `InkWell` (interação idêntica aos cards de lista, #300), centralizados (`WrapAlignment.center`), com alvo de toque ≥48px e sem hover/splash customizados. Cada card exibe **ícone representativo da sessão + rótulo** (parâmetro `icons`, issue #326) — o ícone substitui o `check` no estado concluído; a cor comunica o estado. Estados por preenchimento, com conteúdo (**ícone + texto**) **BRANCO** sobre `primary`/`success` (#294):
  - **Atual/selecionada** = card preenchido `primary` (#FD6B22) + ícone e texto **BRANCOS** (texto em negrito)
  - **Concluída** (wizard) = card preenchido `success` (#4FBF67) + ícone da sessão e texto **BRANCOS**
  - **Inativa/pendente** = card `surface` padrão (borda/elevação) + ícone e texto `textPrimary`
  Sem numeração: o estado é comunicado por cor/ícone. O widget `AppSessionNav` (`flag_core`, parâmetro obrigatório `icons`, com o mesmo tamanho de `sessions`) é usado no wizard (`showDoneState: true`, concluídas em verde) e na tela de detalhe (`showDoneState: false`, apenas atual/inativa). Na tela de detalhe, tocar em um card troca a sessão ativa exibida — a tela mostra **apenas** o conteúdo da sessão ativa.
