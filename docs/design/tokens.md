...
- **Regra de conteúdo sobre primário (#294)**: conteúdo sobre preenchimento `primary` (#FD6B22) e sobre `success` (#4FBF67) usa **BRANCO** — decisão do produto; nota de acessibilidade: contraste branco/laranja ≈ 2,9:1, abaixo de WCAG AA (~4,5:1), aceito conscientemente para estados de seleção
- **Navegação por sessões (#323) — cards centralizados, SEM numeração**: botões de seleção de sessão em formato de `Card` + `InkWell` (interação idêntica aos cards de lista, #300), centralizados (`WrapAlignment.center`), com alvo de toque ≥48px e sem hover/splash customizados. Estados por preenchimento, com conteúdo **BRANCO** sobre `primary`/`success` (#294):
  - **Atual/selecionada** = card preenchido `primary` (#FD6B22) + texto **BRANCO** (negrito)
  - **Concluída** (wizard) = card preenchido `success` (#4FBF67) + ícone check **BRANCO** + texto **BRANCO**
  - **Inativa/pendente** = card `surface` padrão (borda/elevação) + texto `textPrimary`
  Sem numeração: o estado é comunicado por cor/ícone. O widget `AppSessionNav` (`flag_core`) é usado no wizard (`showDoneState: true`, concluídas em verde) e na tela de detalhe (`showDoneState: false`, apenas atual/inativa). O comportamento de rolagem até a seção (`Scrollable.ensureVisible`, ~300ms) e o destaque conforme a rolagem ficam na tela de detalhe.
