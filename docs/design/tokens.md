...
- **Indicador de passos — wizard (padrão de navegação por sessões)**: círculo 28px (`radius 14`), um por etapa, alinhados em linha (`Expanded`), rótulo abaixo do círculo (`FittedBox`). Três estados: **concluído** = círculo `success` (#4FBF67) sólido + ícone de check **branco**; **selecionado (atual)** = círculo `primary` (#FD6B22) sólido + ponto **branco** (ícone `Icons.circle`, 8px); **pendente** = círculo `grayFill` (#F4F5F7) + número ordinal com `textPrimary` (13px). Rótulo da etapa: selecionado = `textPrimary` w700; demais = `textSecondary` w400.
- **Barra de navegação por sessões — tela de detalhe (chips roláveis)**: lista horizontal de chips (raio 10) fixa no topo, refletindo as MESMAS sessões do wizard de cadastro; tocar em um chip rola suavemente até a seção (`Scrollable.ensureVisible`, ~300ms) e o chip da seção visível acompanha a rolagem. Chip **ativo** = fundo `primary` + texto **BRANCO** (#294); chip **inativo** = fundo `grayFill` + texto `textPrimary`. Alvo de toque mínimo ≥48px; interação via `InkWell` padrão do tema, sem hover/splash customizados (#300).
